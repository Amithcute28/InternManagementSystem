...
const { offCampus, interns, files } = defineProps({
        offCampus: Array,
        interns: Array,
        files: Array,
...
const forms = useForm({
  studentId: offCampus?.id,
  studentName: offCampus?.full_name,
  program: offCampus?.program,
  evalForm: null,
});

const submit = () => {
  router.post(route("applications.update", offCampus?.id), {
    _method: "put",
            ...
            <TableDataCell
             ...
             <TableDataCell
              ><div class="flex item-center">
                 ...
                 <Link :href="route('applicationsbsed.updateOffcampus', form.id)" method="PUT" as="button" class="text-green-400 hover:text-red-600">Recommend</Link>
                <Link
                  class="w-4 mr-2 mb-2 transform hover:text-purple-500 hover:scale-110"
                  :href="route('applicationbsed.edit', form.id)"
                >
                  <svg
