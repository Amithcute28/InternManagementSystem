...
watch(search, (value) => {
  router.get('off-campus-applicationbsed', { search: value }, {
    preserveState: true,
    replace: true
  });
});
...
function getTags() {
  router.get('off-campus-applicationbsed', { perPage: perPage.value }, {
    preserveState: true,
    replace: true,
  })
};
...
const submit = () => {
  router.post(route("applicationsbsed.update", props.offCampus?.id), {
    _method: "put",
