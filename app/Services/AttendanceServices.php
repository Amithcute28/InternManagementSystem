...
class AttendanceServices
{
    ...
    public function createAttendance($res, $commonServices): \Illuminate\Http\JsonResponse|\Illuminate\Http\RedirectResponse
    {
        ...
        foreach ($studentIds as $i => $studentId) {
            ...
            $empAtt = User::find($studentId)->attendances()->where('date', $res['date'])->get();
    
            // If there is an attendance record for the employee on the date, update it
            if (count($empAtt) > 0) {
                $empAtt[0]->update([
                    'status' => $res['status'][$i] ?? null,
        ...
        return to_route('attendance-list.attendanceList');
    }

    public function createAttendanceSte($res, $commonServices): \Illuminate\Http\JsonResponse|\Illuminate\Http\RedirectResponse
    {
        $isDayOff = $commonServices->isDayOff($res['date']);
        if ($isDayOff) {
            return response()->json(['message' => 'Cannot create attendance for a day off.']);
        ...
        return to_route('ste-dashboard.create');
    }

    public function deleteDayAttendanceSte($res): \Illuminate\Http\JsonResponse|\Illuminate\Http\RedirectResponse
    {
        ...
        return to_route('ste-attendance.attendanceSte');
    }
...
$currentTimestamp = Carbon::now()->setTimezone('Asia/Manila');
$shiftStartTime = Carbon::createFromFormat('H:i:s', User::find($request->id)->activeShift()->start_time);
$status = ($shiftStartTime->diffInMinutes($currentTimestamp) <= $lateMargin) ? 'on_time' : 'late';

$today = $currentTimestamp->toDateString();
...
// Reset the timezone to the default if needed
$currentTimestamp->setTimezone(config('app.timezone'));
...
return to_route('user.index');
    ...
    public function selfSignOffAttendance($request){
        ...
        $attendance = Attendance::where('student_id', $request->id)->where('date', $today)->first();
